...
@CapabilityDescription("Adds the contents of a FlowFile to a specified array in a MongoDB document")
public class AddToArrayMongo extends AbstractMongoProcessor {
    ...
    static final String MODE_MANY = "many";
    ...
    static final PropertyDescriptor UPDATE_QUERY_KEY = new PropertyDescriptor.Builder()
        ...
        .defaultValue("_id")
        .build();
    static final PropertyDescriptor ARRAY_NAME = new PropertyDescriptor.Builder()
        .name("Name of Array to add to")
    ...
    static {
        ...
        _propertyDescriptors.add(ARRAY_NAME);
...
//            }
            ...
            final String arrayName = context.getProperty(ARRAY_NAME).getValue();
            final Document arrayToAdd = new Document(arrayName, doc.get(arrayName));
            final Document addToArrayOp = new Document("$addToSet", arrayToAdd);
            
            switch(mode) {
                case MODE_SINGLE:
                    collection.updateOne(query, addToArrayOp);
                    break;
                case MODE_MANY:
                    collection.updateMany(query, addToArrayOp);
                    break;
            }
